...
controller.obterUmInterno = async function (id) {
   try {
      const obj = await Tarefa.findById(id)
...
controller.atualizarInterno = async function (id, body) {
   try {
      const obj = await Tarefa.findByIdAndUpdate(id, body)
...
controller.excluir = async function (req, res) {
   try {
      const id = req.body._id
      const obj = await Tarefa.findByIdAndDelete(id)
      if (obj) {
         res.status(204).end()
      }
      else {
...
async function busca(req, res) {
   ...
   try {
      const lista = await Tarefa.find(criterio)
      res.send(lista)
   }
   catch(erro) {
      console.log(erro)
